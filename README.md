...
would perform a simulation study where data are simulated from DGP1 on a $100 \times 100$ grid (resolution $1 \times 1$), with the outcome dependence coming from a GP with a squared exponential covariance that decays to a correlation of $0.1$ when the distance between two units is 10. The seed of the random number generator would initially be set to `0 + sim.i`, where `sim.i` is the $i$th simulation. The number of simulations performed is 25, and for each simulation, the ATE is estimated using a variety of possible estimators. Finally, we simulate from the setting with a linear $\boldsymbol{\mu}$.
